<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none transition ease-in-out duration-150">
                            <div>{{ Auth::guard('mentor')->user()->name }}</div>
                            <div class="ms-1">
                    ...
                    <x-slot name="content">
                        {{-- Mentor Profile (jika ada) --}}
                        {{-- <x-dropdown-link :href="route('mentor.profile.edit')">
                            {{ __('Profile') }}
                        </x-dropdown-link> --}}

                        <form method="POST" action="{{ route('mentor.logout') }}">
                            @csrf
                            <x-dropdown-link :href="route('mentor.logout')"
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            <div class="px-4">
                <div class="font-medium text-base text-gray-800">{{ Auth::guard('mentor')->user()->name }}</div>
                <div class="font-medium text-sm text-gray-500">{{ Auth::guard('mentor')->user()->email }}</div>
            </div>

            <div class="mt-3 space-y-1">
                {{-- <x-responsive-nav-link :href="route('mentor.profile.edit')">
                    {{ __('Profile') }}
                </x-responsive-nav-link> --}}
